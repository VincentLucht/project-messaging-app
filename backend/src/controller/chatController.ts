...
class ChatController {
  createChat = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res.status(500).json({ message: 'Failed to create chat', error: (error as Error).message });
    }
  });
  ...
  addUserToChat = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!otherUser) {
        return res.status(404).json({ message: `Username ${other_username} was not found` });
      }
      ...
      if (memberSet.has(other_username)) {
        return res.status(409).json({ message: `${other_username} is already part of the chat` });
      }
      ...
      return res.status(500).json({ message: 'Failed to add user to chat', error: (error as Error).message });
    }
  });
  ...
  makeUserAdmin = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const otherUser = await db.getUserByUsername(other_username);
      if (!otherUser) {
        return res.status(404).json({ message: `Username ${other_username} not found` });
      }
      ...
      const isOtherUserAdmin = await db.isChatAdminByUsername(chat_id, other_username);
      if (isOtherUserAdmin) {
        return res.status(409).json({ message: `User ${other_username} already is an admin` });
      }

      await db.makeUserAdminByUsername(chat_id, other_username);

      return res.status(201).json({ message: `Successfully made user ${other_username} an admin` });
    } catch (error) {
      return res.status(500).json({ message: 'Failed to make user admin', error: (error as Error).message });
    }
  });
